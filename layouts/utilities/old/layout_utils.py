...
def gridgen(num_points, diameter, min_dist, max_trials=1000):
    # def grid_position(x, y, scale, size):
    #     centre = size / 2
    #     # if size % 2 == 0:
    #     #     centre -= 1
    #     jx = int(round(x * scale)) + centre
    #     jy = int(round(y * scale)) + centre
    #     print('xxx jx', x, size, centre, 1./scale, int(round(x * scale)), '=', jx)
    #     print('xxx jy', y, size, centre, 1./scale, int(round(y * scale)), '=', jy)
    #     return jx, jy

    def grid_position_2(x, y, scale, r):
        jx = int(floor((x + r) * scale))
        jy = int(floor((y + r) * scale))
        # print('xxx jx', x, size, int(round(x * scale)), '=', jx)
        # print('xxx jy', y, size, int(round(y * scale)), '=', jy)
        return jx, jy
    ...
    scale = 1.0 / grid_cell  # Scaling onto the sector grid.
    check_width = 1
    # print('--', size, diameter, min_dist, grid_cell, scale)
    ...
    for j in range(n_req):
        ...
        while not done:
            ...
            # Check if min distance is met.
                ...
                # jx, jy = grid_position(xt, yt, scale, size)
                jx, jy = grid_position_2(xt, yt, scale, r)
                y0 = max(0, jy - check_width)
                y1 = min(grid_size, jy + check_width + 1)
                x0 = max(0, jx - check_width)
                x1 = min(grid_size, jx + check_width + 1)
...
def generate_padded_taylor_weights(diameter, inner_diameter, sll, n=10001):
    n_inner = int(round_to_even((inner_diameter / diameter) * n))
    n_taylor = n - n_inner
    nbar = int(numpy.ceil(2.0 * (numpy.arccosh(10**(-sll / 20.0)) /
                                 numpy.pi)**2 + 0.5))
