...
def plot_psf_2(layouts, freq_hz, fov_deg, size, out_dir):
    if not os.path.isdir(out_dir):
        os.makedirs(out_dir)
    for name in layouts:
        uu = layouts[name]['uu']
        vv = layouts[name]['vv']
        ww = layouts[name]['ww']
        psf = make_psf(uu, vv, ww, freq_hz, fov_deg, size)
        layouts[name]['psf'] = psf
    plot_psf_images_2(layouts, out_dir)
    plot_psf_az_ave_2(layouts, out_dir)
...
def plot_psf_az_ave_2(layouts, out_dir):
    num_bins = 200
    ...
    for name in layouts:
        ...
        y = image.flatten()
        y = y[sort_idx]
        bins, y_db = bin_az_ave_psf(x, y, num_bins, im_size, lm_inc)
        layouts[name]['psf']['bins'] = bins
        layouts[name]['psf']['y_db'] = y_db
    ...
    for i, name in enumerate(layouts):
        bins = layouts[name]['psf']['bins']
        x = bins['x']
        y = bins['mean']
        y_std = bins['std']
        ax.plot(x, y, linestyle='-', color=colors[i], label=name)
        ax.fill_between(x, y - y_std, y + y_std, edgecolor=colors[i],
...
def plot_psf_images_2(layouts, out_dir):
    names = layouts.keys()
    psf0 = layouts[names[0]]['psf']
    psf1 = layouts[names[1]]['psf']
    lm_max = psf0['lm_max']
    lm_inc = psf0['lm_inc']
    fov = psf0['fov']
    off = lm_inc / 2.0
    extent = [-lm_max - off, lm_max - off, -lm_max + off, lm_max + off]
    plot_image_log(psf0['image'], extent, fov, join(out_dir, names[0]))
    plot_image_log(psf1['image'], extent, fov, join(out_dir, names[1]))
