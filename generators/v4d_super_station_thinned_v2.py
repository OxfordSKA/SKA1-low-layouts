...
def gen_super_stations():
    ...
    max_tries_per_station = 10
    diameter_gridgen = 33.0  # m
    diameter = 31.0  # m
    antenna_diameter = 1.5
    num_ant_station = 80
    ss_diameter = 93.0
    st_diameter = diameter_gridgen
    ...
    ss_model_dir = ('v4d_r_90m_%iant_ss_taylor%.1f.tm' % (num_ant_station, sll))
    if os.path.isdir(ss_model_dir):
        shutil.rmtree(ss_model_dir)
    os.makedirs(ss_model_dir)
    st_model_dir = ('v4d_r_90m_%iant_st_taylor%.1f.tm' % (num_ant_station, sll))
    ...
    for i in range(num_super_stations):
        ...
        for j in range(num_stations_per_super_station):
            ...
            while trial < max_tries_per_station:
                print('.', end='')
                ax, ay, mc, _, _, _ = gridgen_taylor(num_ant_station,
                                                     diameter_gridgen,
