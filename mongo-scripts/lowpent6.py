...
def generate_lattice(nx, ny, rmax, sqinc, jitter):
    # TODO-BM add jitter
    ...
    for ix in range(nx):
        for iy in range(ny):
            ...
            xc[ix, iy] += jitter * (2.0 * rand() - 0.5)
            yc[ix, iy] = -rmax + (iy * sqinc * sqrt(3.0) / 2.0)
            yc[ix, iy] += jitter * (2.0 * rand() - 0.5)
...
def generate_ska_low_v4a_super_station(super_station_angle,
    ...
    for j in range(6):
        ...
        xc, yc = generate_lattice(nx, ny, rmax, sqinc, jitter)
...
if __name__ == "__main__":
    ...
    sb6 = numpy.loadtxt('../ant_files/sbfile6.ant')
    ...
    for i in range(plot_num_stations):
        circle = pyplot.Circle((sx0[i], sy0[i]), r_station, color='k',
                               linestyle='--',
    ...
    ax.plot(sb6[:48*6*plot_num_stations, 0],
            sb6[:48*6*plot_num_stations, 1], 'x',
            markeredgecolor='r', markerfacecolor='None')
    for i in range(48*6*plot_num_stations):
        circle = pyplot.Circle((sb6[i, 0], sb6[i, 1]), r_ant, color='r',
                               fill=True, alpha=0.3)
        ax.add_artist(circle)
    ax.set_title('sb6: 1.5m antenna footprints')
